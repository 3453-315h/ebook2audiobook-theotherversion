...
class UnicodeProcessor:
    ...
    def _preprocess_text(self, text: str) -> str:
        # Advanced normalizer for better performance
        # Use NFKC normalization for better compatibility
        text = normalize("NFKC", text)

        # Language detection for proper handling
        try:
        ...
        # Remove emojis (wide Unicode range) - keep this for all languages
        ...
        if is_english:
            # For English, remove only problematic combining diacritics
            text = re.sub(
            ...
            # For non-English languages, preserve diacritics but normalize them
            # Use NFC normalization to compose characters with diacritics
    ...
    def process_text(self, text_list: list[str]) -> tuple[np.ndarray, np.ndarray]:
...
def chunk_text(text: str, max_len: int = 300) -> list[str]:
    """
    Split text into chunks by paragraphs and sentences with advanced processing.
    ...
    # Advanced paragraph splitting - handle various newline formats
    paragraphs = []
    for p in re.split(r"\n\s*\n+", text):
        p = p.strip()
        if p:
            # Handle bullet points and numbered lists
            if re.match(r'^[\*\-\+•\d\w\.\s]+', p):
                # Split list items but keep them together
                paragraphs.append(p)
            else:
                paragraphs.append(p)
    ...
    for paragraph in paragraphs:
        ...
        # Advanced sentence splitting with better abbreviation handling
        # Include more abbreviations and handle edge cases
        abbreviations = r'(?<!Mr\.)(?<!Mrs\.)(?<!Ms\.)(?<!Dr\.)(?<!Prof\.)(?<!Sr\.)(?<!Jr\.)(?<!Ph\.D\.)(?<!etc\.)(?<!e\.g\.)(?<!i\.e\.)(?<!vs\.)(?<!Inc\.)(?<!Ltd\.)(?<!Co\.)(?<!Corp\.)(?<!St\.)(?<!Ave\.)(?<!Blvd\.)(?<!Rd\.)(?<!Ln\.)(?<!Dr\.)(?<!Rev\.)(?<!Gen\.)(?<!Col\.)(?<!Maj\.)(?<!Capt\.)(?<!Sgt\.)(?<!Lt\.)(?<!Cmdr\.)(?<!Adm\.)(?<!Sen\.)(?<!Rep\.)(?<!Gov\.)(?<!Pres\.)(?<!VP\.)(?<!CEO\.)(?<!CFO\.)(?<!CTO\.)(?<!COO\.)(?<!Fig\.)(?<!Eq\.)(?<!No\.)(?<!Jan\.)(?<!Feb\.)(?<!Mar\.)(?<!Apr\.)(?<!Jun\.)(?<!Jul\.)(?<!Aug\.)(?<!Sep\.)(?<!Oct\.)(?<!Nov\.)(?<!Dec\.)(?<!\b[A-Z]\.)(?<=[.!?])\s+'

        sentences = re.split(abbreviations, paragraph)
        ...
        for sentence in sentences:
            ...
            if len(sentence) > max_len * 2:  # If sentence is too long
                # Split at natural breaks (commas, semicolons, etc.)
                sub_sentences = re.split(r'(?<=[,;:]) ', sentence)
                for sub_sentence in sub_sentences:
                    sub_sentence = sub_sentence.strip()
                    if len(current_chunk) + len(sub_sentence) + 1 <= max_len:
                        current_chunk += (" " if current_chunk else "") + sub_sentence
                    else:
                        if current_chunk:
    ...
    # Post-processing: merge very short chunks with next chunk if possible
    final_chunks = []
    i = 0
    while i < len(chunks):
        if i < len(chunks) - 1 and len(chunks[i]) < max_len * 0.3:  # If chunk is very short
            # Try to merge with next chunk if it doesn't exceed max length
            if len(chunks[i]) + len(chunks[i+1]) + 1 <= max_len:
                final_chunks.append(chunks[i] + " " + chunks[i+1])
                i += 2
            else:
...
class TextToSpeech:
    ...
    def _infer(
        ...
        bsz = len(text_list)
        text_ids, text_mask = self.text_processor.process_text(text_list)
