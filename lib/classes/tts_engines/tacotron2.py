...
class Tacotron2TTS:
    ...
    def _load_engine(self) -> None:
        try:
            msg = f"Loading TACOTRON2 {self.tts_key} model, please be patient..."
            print(msg)
            cleanup_memory()
            ...
            if not self.engine:
                if self.session['custom_model'] is not None:
                    ...
                    msg = f"Loading TACOTRON2 custom model..."
                    print(msg)
                    try:
                        # Extract custom model files
                        custom_model_path = self.session['custom_model']
                        required_files = default_engine_settings[TTS_ENGINES['TACOTRON2']]['files']
                        ...
                        if not model_valid:
                            error = f"Custom TACOTRON2 model missing required files: {missing_files}"
                            print(error)
                            ...
                            if sub is not None:
                                ...
                                if self.engine:
                                    # Apply stability optimizations to built-in model
                                ...
                                msg = f"TACOTRON2 checkpoint for {self.session['language']} not found!"
                                print(msg)
                        else:
                            # Use custom model files
                            # TACOTRON2 typically has multiple files
                            model_files = {}
                            found_all = True
                                ...
                                for file in files:
                                    for required_file in required_files:
                                        if file == required_file:
                                            model_files[required_file] = os.path.join(root, file)
                                            break

                            if len(model_files) == len(required_files):
                                model_path = os.path.dirname(next(iter(model_files.values())))
                                self.tts_key = f"{self.session['tts_engine']}-custom-{os.path.basename(custom_model_path)}"
                                self.engine = self._load_api(self.tts_key, model_path, self.session['device'])
                                if self.engine:
                                    # Apply stability optimizations to custom model
                                    m = self.engine.synthesizer.tts_model
                                    d = m.decoder
                                    d.prenet_dropout = 0.0
                                    d.attention_dropout = 0.0
                                    ...
                                    d.attention_keeplast = True
                                if not self.engine:
                                    error = f"Failed to load custom TACOTRON2 model from {model_path}"
                                    print(error)
                                    # Fall back to built-in model
                                    iso_dir = language_tts[self.session['tts_engine']][self.session['language']]
                                    sub_dict = models[self.session['tts_engine']][self.session['fine_tuned']]['sub']
                                ...
                                error = f"Could not find all required model files in custom TACOTRON2 model"
                                print(error)
                                # Fall back to built-in model
                                iso_dir = language_tts[self.session['tts_engine']][self.session['language']]
                                sub_dict = models[self.session['tts_engine']][self.session['fine_tuned']]['sub']
                        ...
                        error = f"Custom TACOTRON2 model loading error: {e}"
                        print(error)
                        # Fall back to built-in model
                        iso_dir = language_tts[self.session['tts_engine']][self.session['language']]
                        sub_dict = models[self.session['tts_engine']][self.session['fine_tuned']]['sub']
                    ...
                    if sub is not None:
                        ...
                        # Apply stability and performance optimizations
                        if self.engine:
                            m = self.engine.synthesizer.tts_model
                            d = m.decoder
            ...
            if self.engine:
                msg = f'TACOTRON2 {self.tts_key} Loaded!'
                print(msg)
        except Exception as e:
    ...
    def convert(self, sentence_index: int, sentence: str) -> bool:
        try:
            ...
            if self.engine:
                ...
                error = f"TACOTRON2 engine could not be loaded!"
                print(error)
                return False
        except Exception as e:
            error = f'Tacotron2TTS.convert(): {e}'
            raise ValueError(e)
            return False