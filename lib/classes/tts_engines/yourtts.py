...
class YourTtsTTS:
    ...
    def _load_engine(self) -> None:
        try:
            msg = f"Loading YOURTTS {self.tts_key} model, please be patient..."
            print(msg)
            cleanup_memory()
            ...
            if not self.engine:
                if self.session['custom_model'] is not None:
                    ...
                    msg = f"Loading YOURTTS custom model..."
                    print(msg)
                    try:
                        # Extract custom model files
                        custom_model_path = self.session['custom_model']
                        required_files = default_engine_settings[TTS_ENGINES['YOURTTS']]['files']
                        ...
                        if not model_valid:
                            error = f"Custom YOURTTS model missing required files: {missing_files}"
                            print(error)
                            ...
                            # YOURTTS typically has a simpler structure
                            model_file = None
                            ...
                            if model_file:
                                ...
                                self.engine = self._load_api(self.tts_key, model_path, self.session['device'])
                                if not self.engine:
                                    error = f"Failed to load custom YOURTTS model from {model_path}"
                                    print(error)
                                    # Fall back to built-in model
                                    model_path = models[self.session['tts_engine']][self.session['fine_tuned']]['repo']
                                    self.tts_key = model_path
                                ...
                                error = f"Could not find model file in custom YOURTTS model"
                                print(error)
                                # Fall back to built-in model
                                model_path = models[self.session['tts_engine']][self.session['fine_tuned']]['repo']
                                self.tts_key = model_path
                        ...
                        error = f"Custom YOURTTS model loading error: {e}"
                        print(error)
                        # Fall back to built-in model
                        model_path = models[self.session['tts_engine']][self.session['fine_tuned']]['repo']
                        self.tts_key = model_path
            ...
            if self.engine:
                msg = f'YOURTTS {self.tts_key} Loaded!'
                print(msg)
        except Exception as e:
        ...
        try:
            ...
            if self.engine:
                ...
                if sentence == TTS_SML['break']:
                    ...
                    language = (self.session['language_iso1'] if self.session['language_iso1'] == 'en' else
                               'fr-fr' if self.session['language_iso1'] == 'fr' else
                               'pt-br' if self.session['language_iso1'] == 'pt' else 'en')
                ...
                error = f"YOURTTS engine could not be loaded!"
                print(error)
                return False
        except Exception as e:
            error = f'YourTtsTTS.convert(): {e}'
            raise ValueError(e)
            return False