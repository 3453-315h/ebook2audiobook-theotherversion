...
# Import the new exception handling and error reporting systems
from lib.classes.exceptions import (
    DependencyError, ProcessingError, FileOperationError, ExceptionHandler,
    safe_execute, TTSEngineError, AudioProcessingError, ValidationError, ErrorSeverity
)
from lib.classes.error_reporter import (
    report_error, wrap_function, wrap_async_function, report_and_continue,
    ErrorReport, ReportDestination, ErrorReporter
)

# Initialize global error reporter
error_reporter = ErrorReporter("ebook2audiobook")

# Remove the old DependencyError class since we're using the new one
...
class SessionTracker:
    ...
    def start_session(self, id:str)->bool:
        with self.lock:
            ...
            if session['status'] is None or session['status'] == 'ready':
...
def prepare_dirs(src:str, id:str)->bool:
    try:
        ...
        # Create directories with specific error handling
        dirs_to_create = [
        ...
        for dir_path in dirs_to_create:
            try:
                ...
                raise FileOperationError(
                    ...
                    context={'directory': dir_path, 'session_id': id}
                )

        session['ebook'] = os.path.join(session['process_dir'], os.path.basename(src))

        # Check for existing ebook with hash comparison
        if os.path.exists(session['ebook']):
            try:
                ...
                raise ProcessingError(
                    message="Failed to compare file hashes during directory preparation",
                    original_exception=e,
                    context={'source': src, 'target': session['ebook']}
                )

        # Clean up old chapters if not resuming
        if not resume:
            try:
                shutil.rmtree(session['chapters_dir'], ignore_errors=True)
            except Exception as e:
                raise FileOperationError(
                    message="Failed to clean up old chapters directory",
                    original_exception=e,
                    context={'directory': session['chapters_dir']}
                )

        # Create chapter directories
        chapter_dirs = [session['chapters_dir'], session['chapters_dir_sentences']]
        for chap_dir in chapter_dirs:
            try:
                os.makedirs(chap_dir, exist_ok=True)
            except OSError as e:
                raise FileOperationError(
                    message=f"Failed to create chapter directory: {chap_dir}",
                    original_exception=e,
                    context={'directory': chap_dir}
                )
        ...
        try:
            ...
            raise FileOperationError(
                message=f"Failed to copy source file from {src} to {session['ebook']}",
                original_exception=e,
                context={'source': src, 'destination': session['ebook']}
            )

        return True

    except FileOperationError:
        # Re-raise file operation errors as they're already properly formatted
        raise
    except ProcessingError:
        # Re-raise processing errors as they're already properly formatted
        raise
    except Exception as e:
...
def check_programs(prog_name:str, command:str, options:str)->bool:
    try:
        ...
        error_msg = f'Error: {prog_name} is not installed! if your OS calibre package version is not compatible you still can run ebook2audiobook.sh (linux/mac) or ebook2audiobook.cmd (windows)'
        DependencyError(
            message=error_msg,
            severity=ErrorSeverity.CRITICAL,
            context={'program': prog_name, 'command': command}
        )
        return False
        ...
        ProcessingError(
            ...
            severity=ErrorSeverity.HIGH,
            original_exception=e,
            context={'program': prog_name, 'command': command, 'options': options}
        )
        return False
    except Exception as e:
        ExceptionHandler.handle_exception(
            e,
            context={'function': 'check_programs', 'prog_name': prog_name, 'command': command}
        )
        return False
    ...
    try:
        ...
        if file_ext == '.pdf':
            ...
            for i, page in enumerate(doc):
                if session.get("cancellation_requested", False):
                    print("OCR cancelled by user.")
                    return False
                try:
                    force_ocr = session.get('force_ocr', False)
                    if force_ocr:
                        text = ''
                    else:
                    ...
                    text = ''
                if not text:
                    reason = "image-based" if not session.get('force_ocr') else "Force OCR enabled"
                    msg = f'The page {i+1} seems to be {reason}. Using OCR...'
                    print(msg)
                    if session['is_gui_process']:
...
def convert_chapters2audio(id:str)->bool:
    ...
    try:
        ...
        tts_manager.setup_performance_optimization()
        resume_chapter = 0
        resume_chapter = 0
        missing_chapters = []
        resume_sentence = 0
            ...
            for x in range(0, total_chapters):
                ...
                for i, sentence in enumerate(sentences):
                    ...
                    if sentence_number in missing_sentences or sentence_number > resume_sentence or (sentence_number == 0 and resume_sentence == 0):
                        ...
                        success = tts_manager.optimize_conversion(sentence_number, sentence) if sentence else True
                                ...
                                if convert2epub(id):
                                    ...
                                    if epubBook:
                                        ...
                                        if is_lang_in_tts_engine:
                                            ...
                                            if session['cover']:
                                                toc, extracted_chapters = get_chapters(epubBook, id)
                                                session['toc'] = toc
                                                if session.get('chapters_updated') and session.get('chapters'):
                                                    print(f"Using {len(session['chapters'])} user-modified chapters. Ignoring {len(extracted_chapters)} extracted chapters.")
                                                else:
                                                    session['chapters'] = extracted_chapters
...
def extract_preview_chapters(file_path:str, id:str, max_chars:int=250, progress=None)->list|None:
    """
    Extract text content from uploaded file for preview in Chapter Editor.
    Splits text into segments optimized for TTS processing based on max_chars.
    Supports: .txt, .epub, .pdf
    Returns a flat list of text segments suitable for the Chapter Editor and TTS.
    
    Args:
        file_path: Path to the uploaded file
        id: Session ID
        max_chars: Maximum characters per segment (default 250, varies by TTS engine)
    """
    ...
    def segment_by_chars(sentences, max_chars):
        """
        Segment sentences to fit within max_chars limit.
        Merges short segments and splits long ones.
        """
        segments = []
        current_segment = ""
        ...
        for sentence in sentences:
            ...
            # If sentence fits in current segment
            if len(current_segment) + len(sentence) + 1 <= max_chars:
                ...
                if len(sentence) > max_chars:
                    # Split long sentence by punctuation or word boundaries
    ...
    try:
        ...
        sentences = []  # Flat list of sentences for TTS
        ...
        if ext == '.txt':
            ...
            try:
                epubBook = epub.read_epub(file_path, options={'ignore_ncx': True})
                spine_ids = [item[0] for item in epubBook.spine]
                all_docs = [
                    item for item in epubBook.get_items_of_type(ebooklib.ITEM_DOCUMENT)
                    if item.id in spine_ids
                ]
                for doc in all_docs:  # Process all documents (no 30 limit)
                ...
                print(f"EPUB preview extraction error: {e}")
                return None
            ...
            try:
                ...
                for page_num in range(total_pages):  # Process all pages (no 30 limit)
                    if session.get("cancellation_requested", False):
                        print("Preview extraction cancelled by user.")
                        return None
                    if progress is not None:
                        progress((page_num + 1) / total_pages, desc=f"Analyzing Page {page_num + 1}/{total_pages}")
                    ...
                    if not text.strip():
                        # Only show alert for first page to avoid spam
                        if page_num == 0:
                            reason = "image-based" if not force_ocr else "Force OCR enabled"
                            msg = f"Preview: {reason} detected. Running OCR on PDF pages..."
                            print(msg)
                            # show_alert({"type": "info", "msg": msg})
                        
                        try:
                            print(f"Preview: OCR processing page {page_num+1}/{len(doc)}")
                            pix = page.get_pixmap(dpi=300)
                            img = Image.open(io.BytesIO(pix.tobytes('png')))
                            # Reuse ocr2xhtml but strip tags to get plain text for the editor
                            xhtml_content = ocr2xhtml(img, session.get('language', 'eng'))
                            if xhtml_content:
                                text = BeautifulSoup(xhtml_content, 'html.parser').get_text(separator=' ', strip=True)
                        except Exception as e:
                            print(f"Preview OCR error on page {page_num+1}: {e}")
                ...
                print(f"PDF preview extraction error: {e}")
                return None
        
        # Apply character-based segmentation
        segments = segment_by_chars(sentences, max_chars)
        
        # No limit on segments - show all document content
        msg = f"Preview ready: {len(segments)} segments extracted."
        print(msg)
        show_alert({"type": "success", "msg": msg})
        
        if segments:
            # Wrap in list to create one "chapter" - convert_chapters2audio expects list of lists
            session['chapters'] = [segments]  # [[seg1, seg2, ...]] instead of [seg1, seg2, ...]
            session['chapters_updated'] = False  # Mark as not user-modified
            return segments
        return None
    except Exception as e:
        print(f"extract_preview_chapters error: {e}")
        return None


def update_session_chapters(id:str, chapters:list)->bool:
    try:
        session = context.get_session(id)
        if session is None:
            error = f'Session {id} not found'
            print(error)
            return False

        # Validate chapters data structure
        if not isinstance(chapters, list):
            error = f'Chapters data must be a list, got {type(chapters)}'
            print(error)
            return False

        # Store the updated chapters in the session
        session['chapters'] = chapters
        session['chapters_updated'] = True
        session['chapters_update_time'] = datetime.now().isoformat()

        msg = f'Successfully updated {len(chapters)} chapters in session {id}'
        print(msg)
        return True

    except Exception as e:
        ExceptionHandler.handle_exception(e, context={'function': 'update_session_chapters', 'session_id': id})
        return False
...
def restore_session_from_data(data:dict, session:dict)->None:
    try:
        ...
        ExceptionHandler.handle_exception(e, context={'function': 'restore_session_from_data'})
