...
for %%p in (%HOST_PROGRAMS%) do (
	...
	if "%%p"=="python" (
		...
		if errorlevel 1 set "prog=python3"
	)
	where /Q !prog!
	if errorlevel 1 (
...
if defined arguments.help (
	...
	if "%SCRIPT_MODE%"=="%BUILD_DOCKER%" (
		if "!DOCKER_DEVICE_STR!"=="" (
			call :check_docker
			if errorlevel 1 goto :failed
			REM docker image inspect "%DOCKER_IMG_NAME%" >nul 2>&1
			REM if errorlevel 0 (
			REM 	echo [STOP] Docker image '%DOCKER_IMG_NAME%' already exists. Aborting build.
			REM 	echo Delete it using: docker rmi %DOCKER_IMG_NAME%
			REM 	goto :failed
			REM )
			REM get deviceinfo via PowerShell
			for /f "delims=" %%a in ('powershell -nologo -noprofile -command ^
				"& { $result = (& { call :check_device_info \"%SCRIPT_MODE%\" } ^| Out-String).Trim(); if ($LASTEXITCODE -ne 0) { exit 1 }; Write-Output $result }"') do (
				set "deviceinfo=%%a"
			)
			if "!deviceinfo!"=="" set "deviceinfo=cpu"
			if errorlevel 1 goto :failed
